...
/**
 * VPNPurchaseScreen - Pantalla profesional de confirmación de compra VPN
 * 
 * Props esperadas via route.params:
 * - paquete: { _id, megas, precio, comentario }
 * - descuentoVPN: Number (descuento del usuario en %)
 */
const VPNPurchaseScreen = ({ route, navigation }) => {
  const { paquete, descuentoVPN = 0 } = route.params || {};
  ...
  useEffect(() => {
    ...
    Meteor.call(
      ...
      (error, result) => {
        setLoading(false);
        if (error) {
          console.error('Error calculando precio VPN:', error);
  ...
  const validarPaqueteActivo = () => {
    ...
    // ✅ user.vpn === true significa VPN activo
    const tieneVPNActivo = user.vpn === true;
    const esIlimitado = user.vpnisIlimitado === true;

    if (tieneVPNActivo && !esIlimitado) {
      Alert.alert(
        'Paquete VPN Activo',
        'Ya tienes un paquete VPN activo. Debes esperar a que venza o se consuma antes de comprar otro.',
        [{ text: 'Entendido' }]
      );
      return false;
    }
  ...
  const procesarCompra = () => {
    ...
    Meteor.call(
      ...
      (error, carritoId) => {
        ...
        if (error) {
          console.error('Error agregando VPN al carrito:', error);
          Alert.alert(
            'Error',
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <Card style={styles.card}>
        <Card.Content>
          <View style={styles.header}>
            <Text variant="headlineSmall" style={styles.titleVPN}>
              Resumen de Compra
            </Text>
            <Chip icon="shield-check" mode="outlined" textStyle={styles.chipVPN}>
              VPN
            </Chip>
          </View>
          ...
          <View style={styles.row}>
            ...
            <Text variant="titleLarge" style={[styles.bold, styles.precioFinalVPN]}>
      ...
      <Card style={styles.card}>
        <Card.Content>
          <Text variant="titleMedium" style={styles.subtitle}>
            Tu Saldo Actual de VPN
          </Text>
          <Divider style={styles.divider} />

          <View style={styles.row}>
            <Text variant="bodyMedium">Estado VPN:</Text>
            {user?.vpn === true ? (
      ...
      <View style={styles.actions}>
        ...
        <Button
          ...
          style={styles.buttonContainedVPN}
          buttonColor="#4CAF50"
          loading={procesando}
          disabled={procesando}
        >
      ...
      <Portal>
        <Dialog visible={dialogVisible} onDismiss={() => setDialogVisible(false)}>
          ...
          <Dialog.Actions>
            ...
            <Button onPress={procesarCompra} mode="contained" buttonColor="#4CAF50">
              Confirmar
            </Button>
          </Dialog.Actions>
        </Dialog>
