...
const BUFFER_OPTIONS = ["--network-caching=12000", "--live-caching=12000", "--file-caching=10000", "--disc-caching=10000", "--http-reconnect", "--avcodec-fast"];
...
const normalizePlaylistUrl = (value, base) => {
  if (typeof value !== "string" || !value.trim()) return null;
  const trimmed = value.trim();
  return /^https?:\/\//i.test(trimmed) ? trimmed : joinUrl(base, trimmed);
};
...
const SeriesPlayer = () => {
  ...
  React.useEffect(() => {
    ...
    const applyStatus = (status) => { if (cancelled) return; const canPlay = status?.playlistReady || status?.ready; if (canPlay) setPlaylistUrl(normalizePlaylistUrl(status.playlistUrl, hlsOrigin) || fallbackPlaylist); if (Number(status?.durationSeconds) > 0) { setDuration(Number(status.durationSeconds) * 1000); durationRef.current = Number(status.durationSeconds) * 1000; } };
    const poll = async () => { if (cancelled) return; try { const response = await fetch(statusUrl, { signal: controller.signal }); const status = await response.json(); if (!response.ok || status?.success === false || status?.status === "error") throw new Error(status?.error || PLAYER_ERROR); applyStatus(status); pollTimerRef.current = setTimeout(poll, status?.status === "ready" ? 10000 : HLS_POLL_MS); } catch (nextError) { if (!cancelled) setStreamError(nextError?.message || PLAYER_ERROR); } };
    setPlaylistUrl(null); setStreamError("");
    fetch(prepareUrl, { method: "POST", signal: controller.signal }).then(async (response) => { const status = await response.json(); if (!response.ok || status?.success === false) throw new Error(status?.error || PLAYER_ERROR); applyStatus(status); pollTimerRef.current = setTimeout(poll, status?.status === "ready" ? 10000 : HLS_POLL_MS); }).catch((nextError) => { if (!cancelled) setStreamError(nextError?.message || PLAYER_ERROR); });
  ...
  const NativePlayer = Platform.OS === "ios" ? AirPlayVideoPlayer : VLCPlayer;
  const playerSource = Platform.OS === "ios" && playlistUrl
    ? { uri: playlistUrl, contentType: "hls" }
    : source;
  const modalCardWidth = Math.min(520, Math.max(280, windowWidth - 48));
  ...
  if (error || !chapter) return <View style={styles.center}><Text style={styles.error}>{error || "El capítulo no está disponible."}</Text><Button mode="contained" onPress={retry}>Reintentar</Button><Button onPress={handleBack}>Volver</Button></View>;
    ...
    return (<><View style={styles.screen}><StatusBar hidden /><View style={styles.videoFrame}>{Platform.OS === "ios" || source ? <NativePlayer key={`${chapterId}:${streamReload}`} ref={playerRef} style={styles.video} useNativeSubtitleMaster={Platform.OS === "ios"} source={playerSource} preparing={Platform.OS === "ios" && !source && !streamError} autoplay paused={paused} startAtSeconds={startAtMs / 1000} autoFullscreen={Platform.OS === "ios"} textTracks={subtitleTextTracks} subtitlesEnabled={externalSubtitleEnabled} subtitleUri={externalSubtitleEnabled ? subtitleUrl : undefined} videoAspectRatio="16:9" resizeMode="contain" acceptInvalidCertificates={false} onLoad={(event) => { const nextDuration = Math.max(durationRef.current, Number(event?.duration || 0)); setDuration(nextDuration); setHasFrame(true); setStreamError(""); revealControls(); if (!viewRegisteredRef.current) { viewRegisteredRef.current = true; addChapterView(chapterId); } }} onProgress={(event) => { const relative = Number(event?.currentTime || 0); const next = Platform.OS === "ios" ? relative : Math.max(0, relative + startAtMs); setCurrentTime(next); if (relative > 0) { setHasFrame(true); } }} onPlaying={() => { setHasFrame(true); revealControls(); }} onPaused={() => { if (Platform.OS !== "ios") setPaused(true); }} onEnd={() => { setPaused(true); saveProgress(chapterId, { completed: true, currentTime: 0, duration: Math.floor(duration / 1000) }, chapter); }} onError={() => { if (hasFrame) setStreamError(PLAYER_ERROR); }} /> : <View style={styles.center}><ActivityIndicator color="#fff" /><Text style={styles.muted}>{streamError || "Preparando streaming seguro..."}</Text></View>}{streamError ? <View style={styles.overlay}><Surface style={styles.errorCard} elevation={3}><Text style={styles.error}>{streamError}</Text><Button mode="contained" onPress={retry}>Reintentar</Button></Surface></View> : null}{controlsVisible ? <View style={[styles.topOverlay, Platform.OS === "ios" && { display: "none" }, { paddingTop: Math.max(insets.top, 10) }]}><IconButton icon="arrow-left" iconColor="#fff" onPress={handleBack} /><View style={styles.topCopy}><Text numberOfLines={1} style={styles.topTitle}>{titleOf(chapter)}</Text><Text numberOfLines={1} style={styles.topMeta}>{series?.nombre || "Series"} · T{season?.numeroTemporada || 1} · E{numberOf(chapter) || "—"}</Text></View></View> : null}<Pressable onPress={() => controlsVisible ? setControlsVisible(false) : revealControls()} style={[styles.tapLayer, Platform.OS === "ios" && { display: "none" }]} accessibilityRole="button" accessibilityLabel="Mostrar controles" />{controlsVisible ? <View style={[styles.controls, Platform.OS === "ios" && { display: "none" }]}><View style={styles.progressRow}><View style={styles.progressTrack}><View style={[styles.progressFill, { width: `${progress * 100}%` }]} /></View><Text style={styles.time}>{formatTime(duration)}</Text></View><View style={styles.controlRow}><IconButton icon={paused ? "play" : "pause"} iconColor="#fff" onPress={() => { setPaused((value) => !value); revealControls(); }} /><IconButton icon="rewind-10" iconColor="#fff" onPress={() => { setStartAtMs(Math.max(0, currentTime - 10000)); setPlaylistUrl(null); setHasFrame(false); setStreamReload((value) => value + 1); revealControls(); }} /><IconButton icon="fast-forward-10" iconColor="#fff" onPress={() => { setStartAtMs(Math.min(duration, currentTime + 10000)); setPlaylistUrl(null); setHasFrame(false); setStreamReload((value) => value + 1); revealControls(); }} /><Text numberOfLines={1} style={styles.controlTitle}>{titleOf(chapter)}</Text><IconButton disabled={!subtitleUrl} icon="subtitles-outline" iconColor="#fff" onPress={() => { setSubtitleDialogVisible(true); revealControls(); }} /></View></View> : null}</View></View><Modal animationType="fade" transparent supportedOrientations={["landscape", "landscape-left", "landscape-right"]} visible={resume !== null} onRequestClose={() => selectResume(false)}><View style={styles.modalBackdrop}><Surface style={[styles.resumeCard, { width: modalCardWidth }]} elevation={4}><Text style={styles.title}>Continuar capítulo</Text><Text style={styles.muted}>Te quedaste en {formatTime(resume?.currentTime || 0)}.</Text><View style={styles.modalActions}><Button onPress={() => selectResume(false)}>Desde el inicio</Button><Button mode="contained" buttonColor="#6366f1" textColor="#fff" onPress={() => selectResume(true)}>Continuar</Button></View></Surface></View></Modal><Modal animationType="fade" transparent supportedOrientations={["landscape", "landscape-left", "landscape-right"]} visible={subtitleDialogVisible} onRequestClose={() => setSubtitleDialogVisible(false)}><View style={styles.modalBackdrop}><Surface style={[styles.resumeCard, { width: modalCardWidth }]} elevation={4}><Text style={styles.title}>Subtítulos</Text>{SUBTITLE_SIZE_OPTIONS.map((option) => <Button key={option.id} mode={option.id === subtitleSizeId ? "contained" : "outlined"} buttonColor={option.id === subtitleSizeId ? "#6366f1" : undefined} textColor="#fff" onPress={() => { setSubtitleSizeId(option.id); setSubtitleDialogVisible(false); }}>{option.label}</Button>)}<Button onPress={() => setSubtitleDialogVisible(false)}>Cerrar</Button></Surface></View></Modal></>);
};

export default SeriesPlayer;

const styles = StyleSheet.create({
  screen: { backgroundColor: "#000", flex: 1 }, videoFrame: { backgroundColor: "#000", flex: 1, overflow: "hidden", position: "relative" }, video: { height: "100%", width: "100%" }, center: { alignItems: "center", backgroundColor: "#020617", flex: 1, gap: 12, justifyContent: "center", padding: 24 }, title: { color: "#fff", fontSize: 23, fontWeight: "900", textAlign: "center" }, muted: { color: "#b8c2d6", fontSize: 16, lineHeight: 23, textAlign: "center" }, error: { color: "#fecdd3", fontSize: 17, lineHeight: 24, textAlign: "center" }, topOverlay: { alignItems: "center", backgroundColor: "rgba(0,0,0,0.42)", flexDirection: "row", left: 0, paddingHorizontal: 10, position: "absolute", right: 0, top: 0, zIndex: 5 }, topCopy: { flex: 1, marginLeft: 4 }, topTitle: { color: "#fff", fontSize: 16, fontWeight: "900" }, topMeta: { color: "rgba(255,255,255,0.7)", fontSize: 12, marginTop: 2 }, tapLayer: { ...StyleSheet.absoluteFill, zIndex: 1 }, controls: { backgroundColor: "rgba(0,0,0,0.5)", bottom: 0, left: 0, paddingBottom: 10, paddingHorizontal: 14, paddingTop: 14, position: "absolute", right: 0, zIndex: 4 }, progressRow: { alignItems: "center", flexDirection: "row", gap: 10 }, progressTrack: { backgroundColor: "rgba(255,255,255,0.35)", borderRadius: 999, flex: 1, height: 3 }, progressFill: { backgroundColor: "#e11d48", borderRadius: 999, height: 3 }, time: { color: "#fff", fontSize: 12, minWidth: 48, textAlign: "right" }, controlRow: { alignItems: "center", flexDirection: "row", minHeight: 50 }, controlTitle: { color: "#fff", flex: 1, fontSize: 14, fontWeight: "800", paddingHorizontal: 8, textAlign: "center" }, overlay: { ...StyleSheet.absoluteFill, alignItems: "center", justifyContent: "center", zIndex: 6 }, errorCard: { alignItems: "center", backgroundColor: "rgba(9,17,31,0.94)", borderRadius: 18, gap: 12, maxWidth: 400, padding: 22 }, modalBackdrop: { alignItems: "center", backgroundColor: "rgba(0,0,0,0.8)", flex: 1, justifyContent: "center", padding: 24 }, resumeCard: { alignSelf: "center", backgroundColor: "#111827", borderColor: "#334155", borderRadius: 18, gap: 12, padding: 24 }, modalActions: { flexDirection: "row", flexWrap: "wrap", gap: 8, justifyContent: "flex-end", marginTop: 8 },
});
