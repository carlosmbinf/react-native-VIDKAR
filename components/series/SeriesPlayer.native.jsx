...
const SeriesPlayer = () => {
  ...
  React.useEffect(() => {
    ...
    const applyStatus = (status) => { if (cancelled) return; if (status?.playlistReady || status?.ready) setPlaylistUrl(normalizePlaylistUrl(status.playlistUrl, hlsOrigin) || fallbackPlaylist); if (Number(status?.durationSeconds) > 0) { setDuration(Number(status.durationSeconds) * 1000); durationRef.current = Number(status.durationSeconds) * 1000; } };
    const poll = async () => { if (cancelled) return; try { const response = await fetch(statusUrl, { signal: controller.signal }); const status = await response.json(); if (!response.ok || status?.success === false || status?.status === "error") throw new Error(status?.error || PLAYER_ERROR); applyStatus(status); pollTimerRef.current = setTimeout(poll, status?.playlistReady ? 10000 : HLS_POLL_MS); } catch (nextError) { if (!cancelled) setStreamError(nextError?.message || PLAYER_ERROR); } };
    setPlaylistUrl(null); setStreamError("");
    fetch(prepareUrl, { method: "POST", signal: controller.signal }).then(async (response) => { const status = await response.json(); if (!response.ok || status?.success === false) throw new Error(status?.error || PLAYER_ERROR); applyStatus(status); pollTimerRef.current = setTimeout(poll, status?.playlistReady ? 10000 : HLS_POLL_MS); }).catch((nextError) => { if (!cancelled) setStreamError(nextError?.message || PLAYER_ERROR); });
